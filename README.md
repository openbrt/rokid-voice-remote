...
不可用，则退回可发现的配对监听状态。已连接主机意外断开时也会尝试一次
恢复，失败后不循环重试。链路密钥仍只由原厂 BSA 数据库保管。
...
- 原厂 BSA HID Device 同一时刻只连接一个主机。默认是将所有指令发给当前
  ...
  方案是只配对一个常在线中控，由中控转发
  CEC/IR/LAN 命令。
- 可以与投影和多台 Google TV 分别完成配对，再把每条语音指令指向命名
  目标；音箱会先切换 HID 主机再发键。所有设备共享 **5 条**语音指令上限，
  并且都必须在蓝牙覆盖范围内。Google TV 型号是否能在待机时被 HID Power 唤醒需要实机验证。
  新增第二台及后续设备前，在 WebUSB 或音箱内置维护页点击“新增设备：进入
  配对模式”，再从目标设备屏幕添加 `Rokid Voice Remote`。
...
```text
http://127.0.0.1:<ADB分配端口>/
```

拔掉 USB 后也可通过 `http://<音箱局域网 IP>:8090/` 访问。内置配置页不再有第二层
登录；它默认把 Wi-Fi 密码保护的家庭局域网作为安全边界。同一局域网内的设备可以
修改语音映射、目标地址并让音箱进入蓝牙配对模式，因此不要把 8090 端口转发到公网，
也不要把音箱放在不可信或共享 Wi-Fi 上。
